...
Model_option = ['yformer','informer','yformer_skipless']
Lr_option=[1e-2,8e-3,5e-3,1e-3,5e-4,1e-4]
Batch_size_op=[64,48,32,16]
# Optim_op=['Adam','SGD','RMSprop']
Optim_op=['Adam',]
input_label_pres_opt=[
    [128,128,32],[128,128,64],
    [144,144,32],[144,144,64],
...
parser.add_argument('--data', type=str, required=False, default='hand_foot', help='data')
...
parser.add_argument('--d_ff', type=int, default=1048, help='dimension of fcn')
...
parser.add_argument('--train_epochs', type=int, default=24888, help='train epochs')
parser.add_argument('--batch_size', type=int, default=64, help='batch size of train input data')
parser.add_argument('--patience', type=int, default=24, help='early stopping patience')
parser.add_argument('--learning_rate', type=float, default=8e-3, help='optimizer learning rate')
parser.add_argument('--des', type=str, default='test',help='exp description')
parser.add_argument('--loss', type=str, default='mse',help='loss function')
parser.add_argument('--lradj', type=str, default='type1',help='adjust learning rate')
parser.add_argument('--optimizer', type=str, default='Adam', help='optimizer of model,choose between [Adam,SGD,RMSProp]')
...
data_parser = {
    'hand_foot':{'data':'case_2_wea.csv', 'T':'OT','M':[7,7,7],'S':[1,1,1],'MS':[7,7,1]},
...
for ii in range(args.itr):
    for input_label_pre_len in input_label_pres_opt:
        args.seq_len,args.label_len, args.pred_len=input_label_pre_len[0],input_label_pre_len[1],input_label_pre_len[2]
        for model in Model_option:
            args.model=model
            for optim in Optim_op:
                args.optimizer=optim
                for bz_idx in Batch_size_op:
                    args.batch_size=bz_idx
